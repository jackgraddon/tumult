                ...
                <template v-if="isLinkActive('/chat')">
                    ...
                    <div v-if="store.invites.length > 0" class="mt-4 flex flex-col gap-2">
                        ...
                        <div 
                            ...
                            :key="invite.roomId"
                            role="button"
                            class="flex items-center gap-2 px-2 py-2.5 rounded-md hover:bg-muted/50 cursor-pointer transition-colors group"
                ...
                <template v-if="isLinkActive('/chat/dms')">
                    ...
                    <div style="content-visibility: auto; contain-intrinsic-size: 0 400px;">
                        <div 
                            ...
                            role="button"
                            class="inline-flex items-center justify-start px-2 h-10 w-full rounded-md text-sm font-medium transition-colors cursor-pointer hover:bg-muted/50 group relative"
                ...
                <template v-if="isLinkActive('/chat/settings')">
                    <div v-for="group in settingsGroups" :key="group.id" class="flex flex-col gap-1 mb-4">
                        ...
                        <div 
                            ...
                            :key="page.path"
                            role="button"
                            class="inline-flex items-center justify-start px-2 h-10 w-full rounded-md text-sm font-medium transition-colors cursor-pointer hover:bg-muted/50"
...
const mapRoom = (room: Room): MappedRoom => {
  ...
  const count = room.getUnreadNotificationCount(store.unreadCountType) ?? 0;
  const manual = store.manualUnread[room.roomId] ? 1 : 0;
